# Building / Compiling ModernItemBlocker

ModernItemBlocker ships as a single source file, `oxide/plugins/ModernItemBlocker.cs`,
...
| Path | Purpose |
| --- | --- |
| `build/ModernItemBlocker.csproj` | SDK-style project that compiles the plugin against the reference assemblies. Targets `net48`. Excludes the bundled `Oxide.References` facade to avoid a `Newtonsoft.Json` `JsonPropertyAttribute` collision (CS0433). |
...
dotnet build build/ModernItemBlocker.csproj -c Release
# or per-build (highest precedence):
dotnet build build/ModernItemBlocker.csproj -c Release -p:ManagedDir="/path/to/.../Managed"
```
